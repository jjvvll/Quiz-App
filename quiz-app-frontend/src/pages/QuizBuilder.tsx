...
interface QuestionItem extends QuestionPayload {
  id: number;
}
...
function createQuestion(type: QuestionType, order: number): QuestionItem {
  return {
    id: ++idCounter,
    quiz_id: 0,
    type,
...
export default function QuestionBuilder() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setQuestions(response.quizData ?? []);
    };
  ...
  const updateChoice = (id: number, idx: number, value: string) => {
    setQuestions((prev) =>
      prev.map((q) => {
        if (q.id !== id) return q;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans px-4 md:px-8 py-6 md:py-10 max-w-3xl mx-auto">
      {/* Header */}
      <div className="flex items-center justify-between mb-8 gap-4">
        <div>
          <p className="text-white/40 text-xs mb-1">Quiz #{quizId}</p>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-zinc-950 text-sm font-medium border-none cursor-pointer bg-gradient-to-br from-amber-400 to-red-500 hover:opacity-90 transition-opacity"
          >
          ...
          {showTypeMenu && (
            <div className="absolute right-0 top-12 w-56 rounded-xl border border-white/10 bg-zinc-900 p-1.5 z-20 shadow-xl">
              {(
                ["multiple_choice", "identification", "essay"] as QuestionType[]
              ).map((type) => (
                <button
                  key={type}
                ...
                >
                  ...
                  <p className="text-white/40 text-xs mt-0.5">
                    {type === "multiple_choice" && "Select one correct answer"}
                  ...
                  {q.options.map((opt, oi) => (
                    <div key={oi} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        value={opt}
                        onChange={(e) => updateChoice(q.id, oi, e.target.value)}
